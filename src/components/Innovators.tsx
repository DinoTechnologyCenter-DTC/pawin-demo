...
const FeatureHighlight: React.FC<{ icon: React.ReactNode; title: string; children: React.ReactNode }> = ({ icon, title, children }) => (
  <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 text-center transform transition-all duration-300 hover:bg-slate-800 hover:-translate-y-2 h-full">
    <div className="bg-gradient-to-r from-amber-500/20 to-orange-500/20 text-amber-400 rounded-lg w-14 h-14 flex items-center justify-center mx-auto mb-5">
      {icon}
    </div>
            ...
            <div className="p-8">
                <div className="flex items-center gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-amber-400 font-semibold">{innovator.project}</p>
                    </div>
                </div>

                <blockquote className="text-slate-300 italic border-l-4 border-amber-500 pl-6 mb-6">
                    {innovator.quote}
                </blockquote>
...
const ProcessStep: React.FC<{ number: string; title: string; children: React.ReactNode; isLast?: boolean }> = ({ number, title, children, isLast = false }) => (
    <div className="relative pl-16 pb-12">
        <div className="absolute left-0 top-0 flex items-center justify-center w-12 h-12 bg-slate-800 border-2 border-amber-500 rounded-full text-lg font-bold text-amber-400">
            {number}
        </div>
    ...
    <div className="bg-slate-900 text-slate-300">
      ...
      <section className="relative pt-32 pb-20 text-center text-white bg-[url('https://picsum.photos/seed/vision/1600/900')] bg-cover bg-center">
        ...
        <div className="relative z-20 container mx-auto px-6">
          ...
          <button onClick={() => openJoinModal('innovator')} className="bg-gradient-to-r from-[#ffae1f] to-[#fe4f51] text-white font-semibold px-8 py-3 rounded-lg hover:opacity-90 transition-all duration-300 transform hover:scale-105">
      ...
      <section className="py-20">
        <div className="container mx-auto px-6">
            <Animated>
              <div className="bg-gradient-to-r from-[#ffae1f] to-[#fe4f51] rounded-xl p-12 text-center text-white relative overflow-hidden">
                   ...
                   <button onClick={() => openJoinModal('innovator')} className="bg-white text-amber-600 font-semibold px-8 py-3 rounded-lg hover:bg-slate-100 transition-all duration-300 transform hover:scale-105">
                      Submit Your Project
                   </button>
              </div>
            </Animated>
