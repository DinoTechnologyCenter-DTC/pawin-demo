...
const buttonVariants = cva(
  [
    "group relative inline-flex items-center justify-center outline-none cursor-pointer rounded-lg font-medium whitespace-nowrap",
    "transition-all duration-300",
    "disabled:opacity-50 disabled:pointer-events-none",
    "focus-visible:ring-2 focus-visible:ring-[#ffae1f]",
  ],
  {
    variants: {
      variant: {
        ...
        tertiary:
          "border border-border text-foreground bg-transparent hover:bg-muted active:scale-95",
        ghost:
          "text-muted-foreground bg-transparent hover:bg-muted hover:text-foreground active:scale-95",
        outline: "border border-input text-foreground bg-transparent hover:bg-accent active:scale-95",
        brand: "bg-[#ffae1f] text-white hover:opacity-90 active:scale-95 shadow-lg shadow-[#ffae1f]/20",
      },
      size: {
            ...
            <>
              {LeadingIcon && (
                <LeadingIcon
                  size={iconSize}
                  strokeWidth={1.5}
                  className="transition-[stroke-width] duration-300 group-hover:stroke-[2]"
                />
              )}
              <span>{children}</span>
              {TrailingIcon && (
