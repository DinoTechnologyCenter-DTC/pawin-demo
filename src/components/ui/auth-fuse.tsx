...
function SignInForm({ setCurrentPage }: { setCurrentPage: (page: string) => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} autoComplete="on" className="flex flex-col gap-4 mt-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid gap-4">
        ...
        <Button type="submit" disabled={loading} variant="brand" size="xl" className="mt-2 h-12 shadow-lg shadow-[#ffae1f]/10">
...
function SignUpForm({ setCurrentPage }: { setCurrentPage: (page: string) => void }) {
  ...
  return (
    <div className="flex flex-col justify-start">
      <StepLabel />
      {error && <div className="p-3 mb-2 bg-red-500/20 border border-red-500/50 rounded-lg text-red-400 text-xs text-center">{error}</div>}
        ...
        {step === 4 && (
          ...
          >
            ...
            <div className="grid gap-3 mt-2">
              ...
              <Button 
                disabled={!formData.selection || loading}
              ...
              <button onClick={handleBack} disabled={loading} className="text-xs text-slate-500 hover:text-slate-300">Go Back</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function AuthFormContainer({ isSignIn, onToggle, setCurrentPage }: { isSignIn: boolean; onToggle: () => void; setCurrentPage: (page: string) => void }) {
    return (
        <div className="mx-auto grid w-full max-w-[360px] gap-0">
            ...
            {isSignIn ? <SignInForm setCurrentPage={setCurrentPage} /> : <SignUpForm setCurrentPage={setCurrentPage} />}
  ...
  return (
    <div className="w-full min-h-screen h-screen flex items-center justify-center bg-slate-900 text-slate-300 overflow-hidden relative">
      ...
      <div className="w-full max-w-lg p-8 md:p-12 relative z-50">
        <AuthFormContainer isSignIn={isSignIn} onToggle={toggleForm} setCurrentPage={setCurrentPage} />
      </div>
